...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    localOrigemSelect.addEventListener('change', atualizarEstoqueDisponivel);
...
// Função para preencher os selects de locais (origem e destino)
function preencherSelectLocais(locais) {
    // Preencher local de origem
...
async function atualizarEstoqueDisponivel() {
    ...
    if (!produtoId || !localOrigemId) {
        estoqueDiv.textContent = 'Selecione produto e local de origem para ver o estoque';
    ...
    try {
        const response = await fetch(`/api/movimentacoes/estoque?produto_id=${produtoId}&local_id=${localOrigemId}`);
        const data = await response.json();
...
function validarLocaisDiferentes() {
    ...
    if (localOrigemId && localDestinoId && localOrigemId === localDestinoId) {
        ...
        localDestinoError.textContent = 'Local de origem e destino devem ser diferentes';
...
async function handleSubmit(event) {
    ...
    const formData = {
        ...
        local_origem_id: parseInt(document.getElementById('localOrigemId').value) || null,
        local_destino_id: parseInt(document.getElementById('localDestinoId').value) || null,
    ...
    try {
        ...
        if (response.ok) {
            ...
            const errorMessage = result.message || result.error || 'Erro ao registrar transferência';
...
function validarFormulario(dados) {
    ...
    if (!dados.local_origem_id) {
        erros.push({ campo: 'localOrigemId', mensagem: 'Local de origem é obrigatório' });
    }
    
    if (!dados.local_destino_id) {
        erros.push({ campo: 'localDestinoId', mensagem: 'Local de destino é obrigatório' });
    }
    
    if (dados.local_origem_id && dados.local_destino_id && dados.local_origem_id === dados.local_destino_id) {
        erros.push({ campo: 'localDestinoId', mensagem: 'Local de origem e destino devem ser diferentes' });
    }
